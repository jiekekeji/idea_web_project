 ...
 */
console.log("myjava.js load end");
...
(function () {
    ...
    app.controller("myjavaCtrl", myjavaCtrlFn);
    function myjavaCtrlFn($http) {
        console.log("myjavaCtrlFn");
        //初始化数据
        self = this;
        http = $http;

        getMyjavaData(page, rows);
        //绑定数据和事件
        //上一页
        self.prePage = function () {
            ...
            getMyjavaData(self.currentPage, rows);
        };
        //下一页
...
// 请求数据填充表单
function getMyjavaData(page, rows) {
    http.jsonp(baseURL + "?page=" + page + "&rows=" + rows + "&id=5&callback=JSON_CALLBACK")
        .success(function (response) {
            console.log("myjava.js load data result");
            console.log("myjava =" + response.status);
